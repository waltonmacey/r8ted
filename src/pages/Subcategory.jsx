...
export default function Subcategory() {
  ...
  return (
    <div className="py-12">
      <p className="eyebrow">
        <Link to="/" className="focus-ring hover:text-on-surface">Home</Link> /{' '}
        <Link to={`/domain/${domainId}`} className="focus-ring hover:text-on-surface">{domain.name}</Link> / {sub.name}
      </p>
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        <h1 className="font-display text-headline-lg-mobile sm:text-headline-lg">{sub.name}</h1>
        {canEdit && (
          <Link
      ...
      {mine.length === 0 ? (
        <p className="mt-12 font-body text-on-surface-variant">
          No lists yet.{' '}
          ...
          {mine.map((l) => {
            ...
            return (
              <Link
                key={l.id}
                to={`/list/${l.id}`}
                className="group focus-ring overflow-hidden rounded border border-outline-variant bg-surface-container transition-colors hover:border-secondary-container"
              >
                {top && (
                  <div className="relative aspect-[16/9] overflow-hidden">
                    <img src={entryImage(top)} alt="" className="img-muted h-full w-full object-cover" loading="lazy" />
                    <span
                ...
                <div className="p-5">
                  ...
                  {l.tagline && <p className="mt-1 font-body text-sm text-on-surface-variant">{l.tagline}</p>}
                  <p className="mt-4 font-mono text-[11px] uppercase tracking-widest text-on-surface-variant">
