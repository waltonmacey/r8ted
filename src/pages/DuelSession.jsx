...
export default function DuelSession() {
  ...
  if (!canEdit)
    return (
      <div className="py-20">
        ...
        <p className="mt-2 font-body text-mute">Unlock edit mode from the nav to run a session.</p>
      </div>
    )
  ...
  return (
    <div className="py-10">
      <p className="eyebrow">
        <Link to={`/list/${list.id}`} className="focus-ring hover:text-paper">
          {list.title}
        </Link>{' '}
        / Duel session
      </p>
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        <h1 className="font-display text-3xl font-bold sm:text-5xl">Duel</h1>
      ...
      {pair.length < 2 ? (
        <div className="mt-14 border border-edge bg-panel p-8 text-center">
          <p className="font-display text-2xl">Everyone is rated.</p>
        ...
        <>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <button
              onClick={() => setRotation((r) => r + 1)}
              className="focus-ring border border-edge px-4 py-2.5 font-ui text-sm hover:border-paper"
            >
              New pairing
            </button>
            <Link to={`/list/${list.id}`} className="focus-ring ml-auto font-mono text-[11px] uppercase tracking-label text-mute hover:text-paper">
              End session
            </Link>
          </div>
        </>
  ...
  return (
    <div className="border border-edge bg-panel">
      <div className="flex gap-4 p-4">
        <img src={entryImage(entry)} alt="" className="img-mono h-28 w-24 shrink-0 object-cover" />
        <div className="min-w-0">
          <p className="eyebrow">{wasRated ? 'Rated anchor' : 'Unrated'}</p>
          <h2 className="mt-1 truncate font-display text-2xl font-semibold">{entry.name}</h2>
          {entry.keyStat && (
            <p className="mt-1 truncate font-mono text-[11px] uppercase tracking-label text-mute">{entry.keyStat}</p>
          )}
        </div>
      </div>
      <div className="space-y-2 border-t border-edge p-4">
        {scoreLabels.map((label, i) => (
          <label key={i} className="flex items-center gap-3">
            <span className="w-24 shrink-0 font-mono text-[10px] uppercase tracking-label text-mute">{label}</span>
            <input
              type="range"
              min="0"
              max="10"
            ...
            <input
              ...
              placeholder="-"
              onChange={(e) => onChange(i, e.target.value)}
              className="focus-ring w-16 border border-edge bg-ink px-2 py-1 text-right font-mono text-sm"
            />
          </label>
        ))}
        <p className="pt-2 font-mono text-xs text-mute">
          {complete ? (
            <>
  ...
  return (
    <section className="mt-10 border-t border-edge pt-5">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {ranked.length === 0 && <span className="font-body text-sm text-mute">No one ranked yet.</span>}
        {ranked.map((e, i) => (
          <span key={e.id} className="contents">
            {i === 8 && (
              <span
                className="mx-1 self-stretch border-l-2 pl-2 font-mono text-[10px] uppercase tracking-label"
            ...
            <span
              className={`flex items-center gap-2 border px-2.5 py-1 font-ui text-xs ${
                activeIds.includes(e.id) ? 'border-paper' : 'border-edge'
              }`}
            >
              <span className="font-bold text-mute">{String(i + 1).padStart(2, '0')}</span>
              <span>{e.name}</span>
        ...
        {benchCount > 0 && (
          <span className="border border-dashed border-edge px-2.5 py-1 font-mono text-[10px] uppercase tracking-label text-mute">
